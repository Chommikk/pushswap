...
void	rthreeb(t_num **a, t_num **b, char **str)
{
	...
	while (i < 10)
	{
		if (sort_check(a, b) == 1)
			return  (ft_putstr_fd(*str, 1), free_arr(a), free_arr(b), free(*str), exit(1));
		if (b[1] != NULL && b[2] != NULL && b[1] ->expected > b[2]->expected && b[2]->expected > b[0]->expected)
			rb(a, b, str);
		else if (b[0] != NULL && b[1] != NULL && b[0]->expected < b[1]->expected)
			sb (a, b, str);
		else if (b[1] != NULL && b[2] != NULL && b[1] ->expected < b[2]->expected)
			rrb(a, b, str);
		i ++;
	}
}
